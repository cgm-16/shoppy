...
export type TimeDealCurrentTabProps = {
  tabNumber: 1 | 2;
  time: number;
};

export const TimeDealCurrentTab = ({
  tabNumber,
  time,
}: TimeDealCurrentTabProps) => {
  ...
  const fetchTimeDeals = async (
    ...
    const res = await fetch(
      `https://assignment-front.ilevit.com/deals/time-deal?page=${pageParam}&time=current`
    );
    return res.json();
  };
  ...
  const {
    ...
    queryKey: ["timeDealCurrentInf"],
    queryFn: (pageParam: number) => fetchTimeDeals(pageParam),
    ...
    getPreviousPageParam: (firstPage, allPages, firstPageParam) => {
      if (isRandomServerErrorType(firstPage) || firstPageParam <= 1) {
        return undefined;
      }
